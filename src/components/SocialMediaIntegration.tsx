...
export default function SocialMediaIntegration({ isAdmin }: SocialMediaIntegrationProps) {
  ...
  const handleUpdatePost = async (id: string, updatedData: Partial<SocialMediaPost>) => {
  ...
  return (
    <div className="p-4">
      <h2 className="text-2xl font-bold mb-4">Intégration des médias sociaux</h2>
      <form onSubmit={handleAddPost} className="mb-4">
        <select
          value={platform}
          onChange={(e) => setPlatform(e.target.value as 'twitter' | 'facebook' | 'instagram')}
      ...
      <ul>
        {posts.map((post) => (
          <li key={post.id} className="mb-2 p-2 border rounded">
            <p><strong>Plateforme:</strong> {post.platform}</p>
            <p><strong>Contenu:</strong> {post.content}</p>
            <p><strong>Date:</strong> {post.date}</p>
            <p><strong>Statut:</strong> {post.status}</p>
            <button
              onClick={() => handleUpdatePost(post.id, { status: post.status === 'draft' ? 'scheduled' : 'published' })}
